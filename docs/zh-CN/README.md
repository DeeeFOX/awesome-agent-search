# 中文文档

[English README](../../README.md) | [中文 README](../../README.zh-CN.md)

这里存放本仓库的中文对照文档。英文文档是 canonical source；中文文档用于中文读者阅读、GitHub 搜索发现，以及 AI agents 用中文检索仓库内容。

## 当前翻译

- [中文搜索发现](discovery.md)
- [最佳实践](best-practices.md)
- [搜索能力接入方案对比](search-integration-strategies.md)
- [SearXNG 搜索后端调研](searxng-search-backend-research.md)
- [Brave Search API 调研](brave-search-api.md)
- [DuckDuckGo Instant Answer API 调研](duckduckgo-instant-answer-api.md)
- [Claude Code MCP 指南](claude-code-mcp.md)
- [Claude Code Bootstrap 要求](claude-code-bootstrap-requirements.md)
- [eze-is/web-access 与浏览器检索](web-access-browser-retrieval.md)
- [仓库曝光清单](visibility-checklist.md)
- [v0.1.0 Release Notes](release-v0.1.0.md)
- [v0.2.0 Release Notes](release-v0.2.0.md)
- [对比调研 Issue Backlog](comparison-issues.md)

## 相关英文文档

- [Best practices](../best-practices.md)
- [Search integration strategies](../comparisons/search-integration-strategies.md)
- [SearXNG search backend research](../research/searxng-search-backend-research.md)
- [Brave Search API research](../research/brave-search-api.md)
- [DuckDuckGo Instant Answer API research](../research/duckduckgo-instant-answer-api.md)
- [Claude Code bootstrap requirements](../integrations/claude-code-bootstrap-requirements.md)
- [Claude Code MCP guide](../integrations/claude-code-mcp.md)
- [eze-is/web-access and Browser Retrieval](../research/web-access-browser-retrieval.md)
- [Repository visibility checklist](../visibility-checklist.md)
- [v0.1.0 release notes](../release-notes/v0.1.0.md)
- [v0.2.0 release notes](../release-notes/v0.2.0.md)
- [Research notes](../research/README.md)
- [Internationalization policy](../i18n.md)

## 维护规则

- 英文文档为准。
- 中文翻译应简洁、准确，不添加英文原文没有的私有部署细节。
- 不翻译、传播或补充任何密钥、cookie、session、私有 endpoint、本机绝对路径或个人邮箱。
- 新增长期可用中文资源时，同步更新 `registry/resources.json`。
